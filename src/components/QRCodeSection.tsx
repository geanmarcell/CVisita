...
export const QRCodeSection: React.FC<QRCodeSectionProps> = ({ data, theme }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-zinc-900 via-zinc-800 to-zinc-900 p-6 sm:p-8 rounded-2xl border border-zinc-700 shadow-xl text-center space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        <button
          ...
          className={`flex items-center gap-3 p-4 rounded-xl font-bold border transition-all cursor-pointer ${
            activeType === 'whatsapp'
        ...
        >
          ...
          <div className="text-left leading-tight">
            <div className="font-extrabold text-xs sm:text-sm md:text-base">WhatsApp</div>
            <div className="text-xs opacity-80">Conversa Direta</div>
          </div>
        </button>
        ...
        >
          ...
          <div className="text-left leading-tight">
            <div className="font-extrabold text-xs sm:text-sm md:text-base">Salvar Contato</div>
            <div className="text-xs opacity-80">vCard Agenda</div>
          </div>
        </button>
        ...
        >
          ...
          <div className="text-left leading-tight">
            <div className="font-extrabold text-xs sm:text-sm md:text-base">Chave PIX</div>
            <div className="text-xs opacity-80">Pagamento</div>
          </div>
        </button>
        ...
        >
          ...
          <div className="text-left leading-tight">
            <div className="font-extrabold text-xs sm:text-sm md:text-base">Link Digital</div>
            <div className="text-xs opacity-80">Cartão Web</div>
          </div>
        </button>
      </div>

      {/* Main Display Box */}
      <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6 sm:p-10 flex flex-col md:flex-row items-center gap-8 sm:gap-10 shadow-2xl">
        ...
        <div className="flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-2xl border-4 border-zinc-800 space-y-3">
          <QRCodeSVG
            ...
            includeMargin={true}
          />
          <span className="text-xs font-mono font-bold text-zinc-700 uppercase tracking-widest">
        ...
        <div className="flex-1 space-y-6 text-left">
          <div className="space-y-2">
            ...
            <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-white">{getQRTitle()}</h3>
          ...
          {activeType === 'pix' && (
            <div className="p-5 rounded-xl bg-amber-500/10 border border-amber-500/30 flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <span className="text-base sm:text-lg font-black text-white font-mono">{data.pixKey}</span>
              </div>
              <button
                onClick={handleCopyRaw}
                className="px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-extrabold text-xs sm:text-sm flex items-center gap-2 cursor-pointer transition-all"
              >
          ...
          {activeType === 'whatsapp' && (
            <div className="p-5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 space-y-2">
              ...
              <p className="text-sm sm:text-base text-emerald-100 italic font-medium">
          ...
          <div className="flex flex-wrap items-center gap-3.5 pt-2">
            <button
              ...
              className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-extrabold text-xs sm:text-sm md:text-base shadow-md transition-all active:scale-95 cursor-pointer"
            >
            ...
            {activeType === 'whatsapp' && (
              <a
                ...
                className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs sm:text-sm md:text-base shadow-md transition-all active:scale-95 cursor-pointer"
              >
            ...
            <button
              onClick={handleCopyRaw}
              className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-extrabold text-xs sm:text-sm md:text-base border border-zinc-700 transition-all active:scale-95 cursor-pointer"
            >
