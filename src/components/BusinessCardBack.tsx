...
export const BusinessCardBack: React.FC<BusinessCardBackProps> = ({
  ...
  return (
    <div
      id="card-back"
      className={`relative w-full aspect-[9/5] rounded-xl sm:rounded-2xl p-5 sm:p-7 flex flex-col justify-between overflow-hidden shadow-2xl transition-all duration-300 ${theme.cardBg} border ${theme.border}`}
    >
      {/* Background elements */}
      {isSophisticated ? (
      ...
      <div className="relative z-10 flex items-center justify-between border-b border-white/10 pb-2.5">
        <div className="flex items-center gap-2">
          <Car className={`w-4 h-4 sm:w-5 sm:h-5 ${theme.accentText}`} />
          <h3 className={`text-xs sm:text-sm font-bold tracking-wide ${theme.textPrimary}`}>
            {data.subtitle}
          </h3>
        </div>
        <div className={`text-[10px] sm:text-xs font-semibold px-2.5 py-0.5 rounded ${theme.badgeBg}`}>
          {data.coverage}
        </div>
      </div>

      {/* Middle Content: Vehicle + Services Grid */}
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-12 gap-3 my-auto py-1.5">
        ...
        <div className="sm:col-span-8 flex flex-col justify-center space-y-2.5">
          ...
          <div className="flex items-center gap-2 text-xs sm:text-sm">
            <span className="text-[9px] uppercase tracking-widest text-white/50 font-bold">Veículo:</span>
            <span className={`font-bold ${theme.accentText}`}>{data.vehicle}</span>
          </div>
        </div>

        {/* Right column - QR Code Contato */}
        {showQrCode && (
          <div className="sm:col-span-4 flex flex-col items-center justify-center p-2.5 rounded-xl bg-white/[0.03] border border-white/10 text-center">
            <div className="p-1 bg-white rounded-md shadow-md mb-1.5">
              <QRCodeSVG
                value={vcardStr}
                size={52}
                bgColor="#FFFFFF"
            ...
            <span className="text-[8px] uppercase tracking-[0.2em] font-semibold text-white/60">
              Escaneie p/ Contato
            </span>
          </div>
        )}
      ...
      <div className="relative z-10 pt-2 border-t border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
        {/* WhatsApp Button Style Contact */}
        <a
          href={waUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm shadow-md transition-all group"
        >
          <MessageCircle className="w-4 h-4 fill-current group-hover:scale-110 transition-transform" />
          <span>{data.phone}</span>
          <span className="text-[10px] bg-emerald-700 px-1.5 py-0.5 rounded text-emerald-100 font-semibold">
            WhatsApp
          </span>
        </a>
